...
- [x] **Design System & Tipografia**: Padronizar paleta de cores (variáveis globais), fontes e espaçamentos no arquivo `tailwind.config.ts`.
...
- [x] **Microinterações e Animações**: Adicionar efeitos de `hover`, transições suaves (`transition-all`), e *toasts/snackbars* para feedback de sucesso ou erro nas ações do usuário.
